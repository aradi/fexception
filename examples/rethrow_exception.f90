...
contains
  ...
  subroutine subroutine_propagating_error(ierror, internal_errorvar)
    ...
    try_catch_env: block
      class(*), allocatable :: new_internal_errorvar
      ...
      try_catch: block
        ...
        type is (linalg_error)
          catch_linalg_error: block
            ...
            print "(a)", "subroutine_propagating_error: rethrowing error"
            new_internal_errorvar = errorvar
            exit try_catch
      ...
      if (allocated(new_internal_errorvar)) then
        call move_alloc(new_internal_errorvar, internal_errorvar)
        return
      end if
...
end module rethrow_exception


program program_rethrow_exception
  use rethrow_exception
  implicit none

  call main()

end program program_rethrow_exception
